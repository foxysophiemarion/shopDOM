...
const itemFetured = document.querySelector('.fetured__body');
	...
	itemFetured.insertAdjacentHTML('beforeend', `
		...
		<div class="item-fetured__body">
			...
			<div class="item-fetured__content">
				<h6 class="item-fetured__title">${element.title}</h6>
				<div class="item-fetured__text">${element.text}</div>
			</div>
			<div class="item-fetured__price"><button type="button">${element.price}</button></div>
		</div>
	</article>
`)
});
...
document.addEventListener('DOMContentLoaded', () => {
	...
	document.querySelectorAll('.item-fetured__cart').forEach(button => {
		button.addEventListener('click', (event) => {
			const item = event.target.closest('.fetured__item');
			addItemToCart(item);
		});
	});

	function addItemToCart(item) {
		const itemTitle = item.querySelector('.item-fetured__title').textContent;
		const itemPrice = item.querySelector('.item-fetured__price button').textContent;

		const cartItem = document.createElement('li');
		cartItem.className = 'cart-item';
		cartItem.innerHTML = `
		  <span>${itemTitle} - ${itemPrice}</span>
